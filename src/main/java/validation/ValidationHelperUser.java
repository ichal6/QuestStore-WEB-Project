...
public class ValidationHelperUser implements ValidationHelper{
    private Validator validator;

    public ValidationHelperUser() {
    ...
    private boolean callEmailValidation(HttpServletRequest request, boolean isInputValid, int min, int max) {
        try {
            validator.validateEmail(request.getParameter("person-mail"), min, max);
